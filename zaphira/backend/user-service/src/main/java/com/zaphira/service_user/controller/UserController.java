...
import com.zaphira.service_user.dto.response.VerifyPinResponse;
import com.zaphira.service_user.dto.request.VerifyPinRequest;
...
import com.zaphira.service_user.services.SecurityQuestionService;
...
//    private final AuthService authService;
    ...
    @Operation(summary = "Get active security questions", description = "Return active security questions ordered by display order")
    public ResponseEntity<ApiResponse<List<SecurityQuestionResponse>>> getSecurityQuestions() {
        List<SecurityQuestionResponse> questions = securityQuestionService.getActiveQuestions();
    ...
    @PostMapping("/{walletId}/pin/verify")
    @Operation(summary = "Verify current PIN", description = "Verify current PIN before changing it")
    public ResponseEntity<ApiResponse<VerifyPinResponse>> verifyCurrentPin(
            @PathVariable Integer walletId,
            @Valid @RequestBody VerifyPinRequest request) {
        VerifyPinResponse response = userService.verifyCurrentPin(walletId.toString(), request);
