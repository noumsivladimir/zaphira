...
//     */
    ...
    @GetMapping("/id/{walletId}/subwallets")
    public ResponseEntity<List<SubWalletResponse>> getSubWalletsByWalletId(@PathVariable Long walletId) {
        log.info("Fetching subwallets for wallet ID: {}", walletId);
        List<SubWalletResponse> subWallets = walletHierarchyService.getSubWalletsByWalletId(walletId);
