...
import com.zaphira.user.kafka.OtpSmsPublisher;
import com.zaphira.user.model.entities.OtpCode;
import com.zaphira.user.model.enums.OtpPurpose;
import com.zaphira.user.repository.OtpCodeRepository;
...
public class OtpServiceImpl implements OtpService {
    ...
    private final com.zaphira.user.client.NotificationServiceClient notificationServiceClient;
    private final OtpSmsPublisher otpSmsPublisher;
    ...
    public OtpServiceImpl(OtpCodeRepository otpCodeRepository, SmsService smsService, EmailService emailService, TelegramService telegramService, com.zaphira.user.client.NotificationServiceClient notificationServiceClient, OtpSmsPublisher otpSmsPublisher) {
        this.otpCodeRepository = otpCodeRepository;
        this.smsService = smsService;
        this.emailService = emailService;
        this.telegramService = telegramService;
        this.notificationServiceClient = notificationServiceClient;
        this.otpSmsPublisher = otpSmsPublisher;
    }
    ...
    public String generateAndSendOtp(String phoneNumber, String email, OtpPurpose purpose) {
        ...
        if (email != null) {
            ...
            // Priorité 1: Utiliser Kafka pour l'envoi asynchrone via notification-service
            if (kafkaEnabled) {
                try {
                    boolean isResend = false; // À déterminer selon le contexte
                    otpSmsPublisher.publishOtpSmsRequest(
                            null, // userId - peut être null si non disponible
                            phoneNumber, 
                            code, 
                            purpose.name(), 
                            OTP_EXPIRY_MINUTES, 
                    ...
                    log.info("OTP SMS request published via Kafka for phone: {}", maskPhoneNumber(phoneNumber));
                } catch (Exception ex) {
                    log.warn("Failed to publish OTP via Kafka, falling back to Feign client for phone: {}", maskPhoneNumber(phoneNumber), ex);
                    sendOtpViaFeignOrDirectSms(phoneNumber, code, purpose);
                }
            } else {
                // Fallback: Feign HTTP ou SMS direct
                sendOtpViaFeignOrDirectSms(phoneNumber, code, purpose);
            }
        }
    ...
    /**
     * Envoie l'OTP via Feign client (notification-service) ou SMS direct en fallback
     */
    private void sendOtpViaFeignOrDirectSms(String phoneNumber, String code, OtpPurpose purpose) {
        try {
            ...
            log.info("OTP sent via notification-service (Feign) for phone: {}", maskPhoneNumber(phoneNumber));
        } catch (Exception ex) {
