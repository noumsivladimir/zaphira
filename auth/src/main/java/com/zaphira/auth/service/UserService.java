...
public class UserService {
        ...
        return userRepository.findByPhoneNumber(request.getPhoneNumber())
                .map(existing -> {
                    ...
                    if (existing.getWalletId() == null) {
                        ...
                        if (wallet != null) {
                            existing.setWalletId(String.valueOf(wallet.getId()));
                            userRepository.save(existing);
                        }
                    }
                    return existing;
                ...
                .orElseGet(() -> {
                    RegularUser newUser = createNewUserWithWallet(request);
                    if (newUser != null) {
                        return newUser;
                    } else {
     ...
     */
    private RegularUser createNewUserWithWallet(RegisterRequest request) {
